...
namespace BrainStew
{
    public static class BLMail
    {
        ...
        public static void SendApprovePayoutMail(string UserName, string TransactionNo, string TransactionDate,string Subject, string MailId)
        {
            MailMessage message = new MailMessage();
            SmtpClient smtp = new SmtpClient();
            string str = string.Empty;
            ...
            using (StreamReader reader = new StreamReader(HttpContext.Current.Server.MapPath("~/congratulation.html")))
            {
                MailText = reader.ReadToEnd();
            }
            MailText = MailText.Replace("[UserName]", UserName);
            MailText = MailText.Replace("[TransactionNo]", TransactionNo);
